...
export default function JetskiForm({ jetski, onSave, onCancel }) {
  ...
  function addOwner() {
    setForm((f) => ({ ...f, proprietarios: [...f.proprietarios, EMPTY_OWNER()] }));
  }

  function removeOwner(idx) {
    setForm((f) => ({ ...f, proprietarios: f.proprietarios.filter((_, i) => i !== idx) }));
  }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.numeroInscricao.trim()) {
      setError('Número de inscrição é obrigatório.');
      return;
    }
    if (!form.proprietarios.length || !form.proprietarios[0].nome.trim()) {
      setError('Pelo menos um proprietário com nome é obrigatório.');
      return;
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-section">
          ...
          <FormGroup legendText="">
            <Checkbox
              id="quadriciclo"
              labelText="Quadriciclo — utiliza o serviço de quadriciclo"
              checked={form.servicos.quadriciclo}
              onChange={() => toggleServico('quadriciclo')}
              disabled={saving}
            />
            <Checkbox
              id="flutuante"
              labelText="Pier flutuante — utiliza o pier flutuante"
              checked={form.servicos.flutuante}
              onChange={() => toggleServico('flutuante')}
              disabled={saving}
            />
          </FormGroup>
        </div>
        ...
        <div className="form-section">
          ...
          {form.proprietarios.map((owner, idx) => (
            <div key={owner.id} className="owner-card">
              <div className="owner-card-header">
                <span style={{ fontSize: '0.875rem', fontWeight: 600, color: '#393939' }}>
                  Proprietário {idx + 1}
                </span>
              ...
              <Stack gap={4}>
                <TextInput
                  ...
                  labelText="Nome *"
                  value={owner.nome}
                  onChange={(e) => setOwnerField(idx, 'nome', e.target.value)}
                  disabled={saving}
                />
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          >
            <Add size={16} /> Adicionar proprietário
          </Button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '1rem', paddingTop: '1rem', borderTop: '1px solid #e0e0e0' }}>
          {saving ? (
            ...
            <>
              <Button type="submit" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
                <Save size={16} /> {isEdit ? 'Salvar alterações' : 'Cadastrar jetski'}
              </Button>
