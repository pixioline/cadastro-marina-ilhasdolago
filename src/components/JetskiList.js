...
export default function JetskiList({ onSelect, onEdit, onNew }) {
  ...
  useEffect(() => {
    const unsub = subscribeJetskis(
      ...
      ()     => {
        setNotification({ kind: 'error', message: 'Erro ao carregar dados do Firebase.' });
        setLoading(false);
      }
    );
    return unsub;
  ...
  const filtered = jetskis.filter((j) => {
    ...
    return (
      ...
      j.proprietarios?.some(
        (p) => p.nome?.toLowerCase().includes(q) || p.apartamento?.toLowerCase().includes(q)
      )
    );
  });
  ...
  return (
    <div>
      <div className="header-row">
        ...
        <Button onClick={onNew}><Add style={{ marginRight: '0.5rem' }} /> Novo Jetski</Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          {!search && (
            <Button onClick={onNew} style={{ marginTop: '1rem' }}>
              <Add style={{ marginRight: '0.5rem' }} /> Cadastrar primeiro jetski
            </Button>
          )}
        </div>
      ) : (
    ...
    <Tile style={{ padding: 0, overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ padding: '1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <p style={{ margin: 0, fontSize: '0.75rem', color: '#6f6f6f', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              Nº Inscrição
            </p>
            <p style={{ margin: '0 0 0.25rem', fontSize: '1.125rem', fontWeight: 600, color: '#161616' }}>
        ...
        <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem' }}>
          <Button kind="ghost" size="sm" onClick={onView}><View style={{ marginRight: '0.375rem' }} /> Ver detalhes</Button>
          <Button kind="ghost" size="sm" onClick={onEdit}><Edit style={{ marginRight: '0.375rem' }} /> Editar</Button>
        </div>
      </div>
    </Tile>
  );
